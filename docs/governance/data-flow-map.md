...
| 1c  | Trust S3 (diagnostics_orders, providers)                                  | ECS Fargate (`ingest-trust-s3`)                    | HTTPS (S3 API)              | SSE-S3 (Trust-side)                                            | None (raw)                                                                                                                 | Cross-account S3 bucket policy grant (read-only)                                                            |
| 2   | ECS Fargate (all three tasks)                                             | S3 Bronze (Platform data lake)                     | HTTPS (S3 API)              | KMS CMK (customer-managed, per-env key, S3 Bucket Key enabled) | None (raw Parquet)                                                                                                         | ECS task role: write `bronze/*` + `_manifests/*` prefixes only                                              |
...
| 4   | Spectrum Bronze external tables                                           | Redshift Silver schema (internal tables)           | Internal Redshift           | Redshift namespace KMS CMK                                     | **HMAC-SHA-256** via Lambda UDF (per-env Secrets Manager key). NHS Mod-11 validation; failed records routed to quarantine. | dbt execution role: write `silver` schema                                                                   |
...
| 6   | Redshift Gold tables                                                      | S3 Gold export (Platform data lake)                | HTTPS (S3 API, `UNLOAD`)    | KMS CMK (same lake key)                                        | Inherited (`patient_sk`)                                                                                                   | Pipeline task role: write `gold/*` prefix                                                                   |
| 7   | S3 Gold export                                                            | Streamlit Community Cloud (dashboard)              | HTTPS (S3 API, `GetObject`) | KMS CMK at rest; TLS in transit                                | Inherited (`patient_sk`). Small-cell suppression applied at Gold layer (counts < 5 suppressed).                            | IAM user: read-only on `gold/*` prefix                                                                      |

...
{#
    ...
    Key decisions: D-05 (per-referral grain), D-14 (distkey/sortkey), D-17 (treatment_month)
#}
...
first_treatment AS (
    ...
    FROM (
        SELECT
            r.referral_id,
            e.encounter_datetime_start::date AS treatment_date,
            e.wait_time_days                 AS enc_wait_time_days,
            ROW_NUMBER() OVER (PARTITION BY r.referral_id ORDER BY e.encounter_datetime_start) AS rn
        FROM referrals r
...
SELECT
    ...
    ft.treatment_date,
    COALESCE(ft.enc_wait_time_days, DATEDIFF('day', r.referral_datetime::date, ft.treatment_date)) AS wait_days,
    CASE
        WHEN COALESCE(ft.enc_wait_time_days, DATEDIFF('day', r.referral_datetime::date, ft.treatment_date)) > 126
    ...
    -- Time-series: treatment month (completed pathway view, NHS RTT standard)
    DATE_TRUNC('month', ft.treatment_date)::date                        AS treatment_month,

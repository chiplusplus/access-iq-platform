# ADR-005: Data Quality Scope -- dbt-expectations Default + Selective Great Expectations
...
Phase 6 requires a data quality framework that (a) validates all Silver and Gold models
for structural integrity and (b) gates Gold promotion on person-level Silver data quality.

The original REQ-DQ-01 called for Great Expectations validation suites across all Silver tables.
Decision D3 (PROJECT.md) softened this: GE is narrowly scoped, with dbt-expectations as the
broad default layer.
...
- Person-level tables (patients, encounters, referrals, diagnoses) carry highest data sensitivity
  and are the primary join keys for all downstream analytics
...
### Known limitation: silver.diagnoses is empty

The `diagnoses` Silver table is currently empty due to a simulator bug (northshire-hospital-sim
does not populate the diagnoses export). GE validates diagnoses with `min_value=0` for
`ExpectTableRowCountToBeBetween`, which means GE silently passes on this table. This reduces
the DQ signal for diagnoses until the simulator bug is fixed. When the simulator is updated to
produce diagnoses data, update `min_value` to `1` in `run_ge_gate.py` (line in
`build_suite_for_table` under `elif table_name == "diagnoses"`).
